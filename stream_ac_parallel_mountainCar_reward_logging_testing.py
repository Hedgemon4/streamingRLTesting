...
def main(seed, observation_delay=0, repeat_interval=0, debug=False):
    ...
    other_returns, other_term_time_steps = [], []
    counter = 0
    s, _ = env.reset(seed=seed)
    timestep = 0
    other_returns.append(0)
    other_term_time_steps.append(0)
    for t in range(1, num_training_timesteps+1):
        ...
        s = s_prime
        other_returns[counter] += r * (gamma ** counter)
        timestep += 1
        if terminated or truncated:
            ...
            s, _ = env.reset()
            other_term_time_steps[counter] = timestep
            timestep = 0
            counter += 1
            other_returns.append(0)
            other_term_time_steps.append(0)
    ...
    env.close()
    save_dir = "mountainCar/data_stream_ac_reward_difference_testing/delay_{}/interval_{}/seed_{}".format( observation_delay, repeat_interval, seed)
    ...
    with open(os.path.join(save_dir, "other_training_data.pkl".format(seed)), "wb") as f:
        pickle.dump((other_returns, other_term_time_steps, env_name), f)
